    ...
    export default {
        ...
        methods: {
            ...
            loadNode(node, resolve){
                ...
                // console.log(resolve)
                if (!node.data.pathName || node.data.file) {
                    return resolve([])
            ...
            getTreeChild(data, resolve) {
                ...
                api.post("sys/findFileList", { userId: this.user.id,ftpPath: data.pathName+"/",
                    ...
                    if (res.code == 2000) {
                        _self.childrenList = res.data[0].ftpFile;
                        _self.recursion(_self.childrenList, true);
                        _self.queryNodeByPathName(_self.baseList,data.pathName,_self.childrenList);
                        return resolve(_self.childrenList);
                    }
                });
            },
            queryNodeByPathName(list,pathName,childList){
                list.forEach(v => {
                    if (v.pathName == pathName) {
                        v.children = childList;
                    }
                    if (v.children) {
                        this.queryNodeByPathName(v.children,pathName,childList);
                    }
                });
            }
        },
