...
export default {
  data() {
    return {
      ...
      list: [],
      baseList: [],
      metas: ["avi", "wmv", "mpeg", "mp4", "m4v", "mov", "asf", "flv", "f4v", "rmvb", "rm", "3gp", "vob", "mkv"],
      isVideo: false,
      video: {
        isShow: false,
        src: ''
  ...
  methods: {
    ...
    getQuery(list) {
      list.forEach(v => {
        if (v.file) {
          if (this.query.date && this.query.name) {
            if (v.fileName.includes(this.query.name) && (v.date > new Date(this.query.date[0] + ' 00:00:01').getTime() && v.date < new Date(this.query.date[1] + ' 23:59:59').getTime())) {
              this.list.push(v);
            }
          } else if (this.query.date && !this.query.name) {
            if (v.date > new Date(this.query.date[0] + ' 00:00:01').getTime() && v.date < new Date(this.query.date[1] + ' 23:59:59').getTime()) {
              this.list.push(v);
            }
          } else if (!this.query.date && this.query.name) {
    ...
    setSort(parent, list) {
      if (list.length) {
        ...
        if (this.query.filesort == 1) {
          file.sort((a, b) => { return a.py.localeCompare(b.py) })
          text.sort((a, b) => { return a.py.localeCompare(b.py) })
        } else if (this.query.filesort == 2) {
          file.sort((a, b) => { return b.py.localeCompare(a.py) })
          text.sort((a, b) => { return b.py.localeCompare(a.py) })
        }

        if (this.query.timesort == 1) {
          file.sort((a, b) => { return a.date - b.date })
          text.sort((a, b) => { return a.date - b.date })
        } else if (this.query.timesort == 2) {
          file.sort((a, b) => { return b.date - a.date })
          text.sort((a, b) => { return b.date - a.date })
        }
        list = [];
